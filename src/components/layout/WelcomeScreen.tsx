...
export function WelcomeScreen() {
  ...
  useEffect(() => {
    if (sessionStorage.getItem("blessing-welcomed")) return;
    // eslint-disable-next-line react-hooks/set-state-in-effect -- first-visit gate
    setShow(true);
    ...
    const interval = setInterval(() => {
      ...
      if (elapsed >= WELCOME_DURATION_MS) {
        ...
        sessionStorage.setItem("blessing-welcomed", "1");
        setShow(false);
    ...
    <AnimatePresence>
      {show && (
        <motion.div
          ...
          className="fixed inset-0 z-[250] flex flex-col items-center justify-center bg-cream"
          role="dialog"
